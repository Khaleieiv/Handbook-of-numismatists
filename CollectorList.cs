...
{
    public class CollectorList : List<Сollector> 
    {
           TxtReader txt;
        ...
        public void ChangeElem(int coinindex, string n, string y, string c, string d)//меняет элемент коллекции
        {
            ...
            Сollector current = this[coinindex];
        ...
        {
            int n = 0;
            foreach (Сollector x in this)
            {
                Console.WriteLine(n + ":\t " + x.Name + ", " + x.Country + ", " + x.Details + ", " + x.Rarecoin);
                n++;
            }
        }
